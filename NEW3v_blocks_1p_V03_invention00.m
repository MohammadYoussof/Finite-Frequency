...
colormap(cmap);
load figures/plot_pts_syn_mf_invention.mat

plot_pt_syn_mf_invention.lat = plot_pts_syn_mf_invention.lat';
plot_pt_syn_mf_invention.lon = plot_pts_syn_mf_invention.lon';
plot_pt_syn_mf_invention.z = plot_pts_syn_mf_invention.z';
plot_pt_syn_mf_invention.val = plot_pts_syn_mf_invention.val';
...
for kk = 1:size(A,1)
    ...
    plot_pt_syn_mf_invention.dist1 = distance( plot_pt_syn_mf_invention.lat, plot_pt_syn_mf_invention.lon, ...
                    A(kk,2).*ones(size(plot_pt_syn_mf_invention.lat)), A(kk,1).*ones(size(plot_pt_syn_mf_invention.lat)) );
    plot_pt_syn_mf_invention.dist2 = distance( plot_pt_syn_mf_invention.lat, plot_pt_syn_mf_invention.lon, ...
                    B(kk,2).*ones(size(plot_pt_syn_mf_invention.lat)), B(kk,1).*ones(size(plot_pt_syn_mf_invention.lat)) );
    xd = distance( A(kk,2),A(kk,1),B(kk,2),B(kk,1) );       % define an along Xsection distance
    xdr=(pi/180)*xd;
    plot_pt_syn_mf_invention.dist12 = plot_pt_syn_mf_invention.dist1 + plot_pt_syn_mf_invention.dist2;
    sb_indx = find(plot_pt_syn_mf_invention.dist12 < (xd+0.35) );
    xxx = plot_pt_syn_mf_invention.lon(sb_indx);
    yyy = plot_pt_syn_mf_invention.lat(sb_indx);
    zzz = -plot_pt_syn_mf_invention.z(sb_indx);
    vp  = -plot_pt_syn_mf_invention.val(sb_indx); % there's an issue here with the sign
    %---i changed this vp myslef...remember ya mohamed%%%%
    %%%vp  = plot_pt_syn_mf_invention.val(sb_indx); % use this for preferred model, hte previous one for synth
...
set(gcf,'units',get(gcf,'PaperUnits'),'Position',get(gcf,'PaperPosition'))
    set(gca,'clim', [-0.8 0.8])
     ...
     %print(['figures/blocks_mf_model_inventoin_a.pdf'],'-dpdf','-r950');
     
    
end

save newblocks_synth1