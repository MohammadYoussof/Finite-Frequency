...
colormap(cmap);
load figures/plot_pts_syn_mf_crt.mat

plot_pt_syn_mf_crt.lat = plot_pts_syn_mf_crt.lat';
plot_pt_syn_mf_crt.lon = plot_pts_syn_mf_crt.lon';
plot_pt_syn_mf_crt.z = plot_pts_syn_mf_crt.z';
plot_pt_syn_mf_crt.val = plot_pts_syn_mf_crt.val';
...
for kk = 1:size(A,1)
    ...
    plot_pt_syn_mf_crt.dist1 = distance( plot_pt_syn_mf_crt.lat, plot_pt_syn_mf_crt.lon, ...
                    A(kk,2).*ones(size(plot_pt_syn_mf_crt.lat)), A(kk,1).*ones(size(plot_pt_syn_mf_crt.lat)) );
    plot_pt_syn_mf_crt.dist2 = distance( plot_pt_syn_mf_crt.lat, plot_pt_syn_mf_crt.lon, ...
                    B(kk,2).*ones(size(plot_pt_syn_mf_crt.lat)), B(kk,1).*ones(size(plot_pt_syn_mf_crt.lat)) );
    xd = distance( A(kk,2),A(kk,1),B(kk,2),B(kk,1) );       % define an along Xsection distance
    xdr=(pi/180)*xd;
    plot_pt_syn_mf_crt.dist12 = plot_pt_syn_mf_crt.dist1 + plot_pt_syn_mf_crt.dist2;
    sb_indx = find(plot_pt_syn_mf_crt.dist12 < (xd+0.35) );
    xxx = plot_pt_syn_mf_crt.lon(sb_indx);
    yyy = plot_pt_syn_mf_crt.lat(sb_indx);
    zzz = -plot_pt_syn_mf_crt.z(sb_indx);
    vp  = -plot_pt_syn_mf_crt.val(sb_indx); % there's an issue here with the sign
    %---i changed this vp myslef...remember ya mohamed%%%%
    %%%vp  = plot_pt_syn_mf_crt.val(sb_indx); % use this for preferred model, hte previous one for synth
...
set(gcf,'units',get(gcf,'PaperUnits'),'Position',get(gcf,'PaperPosition'))
    set(gca,'clim', [-0.8 0.8])
     ...
     %print(['figures/P_blocks_vert_synth.pdf'],'-dpdf','-r950');
     
    
end
save blocking_synth1