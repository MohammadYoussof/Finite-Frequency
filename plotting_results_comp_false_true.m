...
load figures_true_ref/Namib_True_Ref.mat
vm_true=vm;
s_residuals_true=stn.resid;

%difference
vm_comp=vm_true-vm_false;
s_residuals_comp=s_residuals_true-s_residuals_false;

figure(1)

if plot_tomo_maps == true
   
    make_DS_map_comp(vm_comp,par,stax,stay);
end
...
% s_residuals_false=stn.resid;
 plot_s_resids_comp(par, data, s_residuals_comp);  hold on; set(gca,'clim', [-0.5 0.5])
   cs = max(abs([-0.8, 0.8]));
  print(['figures_comp_ref/stns_residuals_comp.pdf'],'-dpdf','-r500');
