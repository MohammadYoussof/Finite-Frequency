...
colormap(cmap);
load figures/plot_pts_syn_new_m_invent.mat

plot_pt_syn_new_m_invent.lat = plot_pts_syn_new_m_invent.lat';
plot_pt_syn_new_m_invent.lon = plot_pts_syn_new_m_invent.lon';
plot_pt_syn_new_m_invent.z = plot_pts_syn_new_m_invent.z';
plot_pt_syn_new_m_invent.val = plot_pts_syn_new_m_invent.val';
...
for kk = 1:size(A,1)
    ...
    plot_pt_syn_new_m_invent.dist1 = distance( plot_pt_syn_new_m_invent.lat, plot_pt_syn_new_m_invent.lon, ...
                    A(kk,2).*ones(size(plot_pt_syn_new_m_invent.lat)), A(kk,1).*ones(size(plot_pt_syn_new_m_invent.lat)) );
    plot_pt_syn_new_m_invent.dist2 = distance( plot_pt_syn_new_m_invent.lat, plot_pt_syn_new_m_invent.lon, ...
                    B(kk,2).*ones(size(plot_pt_syn_new_m_invent.lat)), B(kk,1).*ones(size(plot_pt_syn_new_m_invent.lat)) );
    xd = distance( A(kk,2),A(kk,1),B(kk,2),B(kk,1) );       % define an along Xsection distance
    xdr=(pi/180)*xd;
    plot_pt_syn_new_m_invent.dist12 = plot_pt_syn_new_m_invent.dist1 + plot_pt_syn_new_m_invent.dist2;
    sb_indx = find(plot_pt_syn_new_m_invent.dist12 < (xd+0.35) );
    xxx = plot_pt_syn_new_m_invent.lon(sb_indx);
    yyy = plot_pt_syn_new_m_invent.lat(sb_indx);
    zzz = -plot_pt_syn_new_m_invent.z(sb_indx);
    vp  = -plot_pt_syn_new_m_invent.val(sb_indx); % there's an issue here with the sign
    %---i changed this vp myslef...remember ya mohamed%%%%
    %%%vp  = plot_pt_syn_new_m_invent.val(sb_indx); % use this for preferred model, hte previous one for synth
...
set(gcf,'units',get(gcf,'PaperUnits'),'Position',get(gcf,'PaperPosition'))
    set(gca,'clim', [-0.8 0.8])
     ...
     print(['figures/P_blocks_new_m_model_syntha.pdf'],'-dpdf','-r950');
