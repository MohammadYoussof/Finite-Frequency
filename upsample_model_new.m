...
plotx = modx(2):15:modx(end-1); ploty = mody(end-1):15:mody(2); plotz = modz(2:end-1);
...
for ii = 1:(length(plot_z)+length(plotz))
    ...
    plot_pt(ii).V = zeros(size(plot_pt(1).X));
end
...
for iz = 1:length(plot_z)
    ...
    plot_pt(q).V = (up_wt).*plot_pt(mz_up).val + (dn_wt).*plot_pt(mz_dn).val;
    q=q+1;
end
...
for ii = 1:length(plot_z)+length(plotz)
    ...
    plot_pts.V = [ plot_pts.V ; plot_pt(ii).V ];
end  
